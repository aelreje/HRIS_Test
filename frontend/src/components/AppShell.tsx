...
export const AppShell = ({ children, roleId, onLogout }: { children: React.ReactNode; roleId: number; onLogout: () => void }) => {
  ...
  const filteredNavItems = useMemo(() => {
    return NAV_ITEMS.filter(item => !item.roleRequired || roleId >= item.roleRequired);
  }, [roleId]);
  ...
  return (
    <div className="flex h-screen bg-[#ECEFF1] font-['Roboto',sans-serif] antialiased overflow-hidden relative">
      ...
      >
        ...
        {/* Profile Section */}
        <header className={cn(
          "h-[82px] flex items-center bg-[#E3F2FD] mb-[48px] transition-all duration-300 shrink-0",
          isCollapsed ? "px-0 justify-center" : "px-[36px] gap-[22px]"
        )}>
          <div className={cn(
            "rounded-full bg-[#91D5FF] text-[#096DD9] flex items-center justify-center font-normal flex-shrink-0 transition-all shadow-sm",
          ...
          {!isCollapsed && (
            <div className="flex flex-col whitespace-nowrap overflow-hidden">
              <h2 className="text-[20px] font-normal text-[#263238] leading-[32px] tracking-[0.25px]">User Name</h2>
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300 relative">
        {/* Top Header Bar (from Figma 2096:2664) */}
        ...
        {/* Content Box (from Figma 2096:2662) */}
        <main className="flex-1 overflow-y-auto flex justify-center py-0">
          <div className="w-full lg:w-[984px] min-h-full bg-[#F5F5F5] shadow-[0px_1px_1px_0px_rgba(0,0,0,0.2),0px_2px_2px_0px_rgba(0,0,0,0.14),0px_1px_5px_0px_rgba(0,0,0,0.12)] transition-all duration-300">
            <div className="p-10">
              {children}
            </div>
          </div>
